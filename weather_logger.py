    ...
    try:
        ...
        if response.status_code == 200: #client request received/processed by server
            return {
                ...
                "gust_mph": data["wind"].get("gust", "NA"), # gusts (if available, not for me where I am)
...
def main():
    ...
    while True:
        ...
        if weather_data:
            ...
            print(f"[{weather_data['timestamp']}] Wind: {weather_data['wind_speed_mph']} mph | Dir: {weather_data['wind_deg']}° | Humidity: {weather_data['humidity']}%")
